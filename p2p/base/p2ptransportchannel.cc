  ...
  if (allocator_sessions_.empty() ||
    ...
    if (pooled_session) {
      ...
      AddAllocatorSession(allocator_->CreateSession(
          ...
          ice_parameters_.pwd));
	  ALOGE_PP("ppt, in P2PTransportChannel::MaybeStartGathering, go to StartGettingPorts.\n");
...
void P2PTransportChannel::OnUnknownAddress(
  ...
  RTC_LOG(LS_ERROR) << "ppt, in P2PTransportChannel::OnUnknownAddress, Adding connection from "
...
bool P2PTransportChannel::CreateConnection(PortInterface* port,
                                           ...
                                           PortInterface* origin_port) {
  RTC_LOG(LS_ERROR) << "ppt, in P2PTransportChannel::CreateConnection.\n";
  if (!port->SupportsProtocol(remote_candidate.protocol())) {
    return false;
  }
  /*
  if(strstr(remote_candidate.ToString().c_str(), ".149") == NULL)
  {
      return false;
  }
  */
  ...
  if (connection == nullptr ||
	...
	RTC_LOG(LS_ERROR) << "ppt, in P2PTransportChannel::CreateConnection, Adding connection from "
                   << " candidate: " << remote_candidate.ToString();
    AddConnection(connection);
...
void P2PTransportChannel::OnReadPacket(Connection* connection,
  ...
  ALOGI("ppt, in P2PTransportChannel::OnReadPacket, go to SignalReadPacket, len = %d.\n", len);
