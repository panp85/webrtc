...
bool RtpDemuxer::AddSink(uint32_t ssrc, RtpPacketSinkInterface* sink) {
  ...
  ALOGI("ppt, in RtpDemuxer::AddSink, go to AddSink 1.\n");
...
void RtpDemuxer::AddSink(const std::string& rsid,
  ...
  RtpDemuxerCriteria criteria;
  criteria.rsid = rsid;
  ALOGI("ppt, in RtpDemuxer::AddSink, go to AddSink 2.\n");
...
bool RtpDemuxer::OnRtpPacket(const RtpPacketReceived& packet) {
  ...
  if (sink != nullptr) {
  	ALOGI("ppt, in RtpDemuxer::OnRtpPacket, go to sink->OnRtpPacket, sink: %p.\n", sink);
...
RtpPacketSinkInterface* RtpDemuxer::ResolveSink(
  ...
  ALOGI("ppt, in RtpDemuxer::ResolveSink, ssrc = %u, has_mid = %d, has_rsid = %d.\n", ssrc, has_mid, has_rsid);
  ...
  ALOGI("ppt, in RtpDemuxer::ResolveSink, rsid: %s, mid: %s.\n", rsid?rsid->c_str():"no", mid?mid->c_str():"no");
  ...
  if (ssrc_sink_it != sink_by_ssrc_.end()) {
  	ALOGI("ppt, in RtpDemuxer::ResolveSink, return ssrc_sink_it->second: %p.\n", ssrc_sink_it->second);
    return ssrc_sink_it->second;
  }
  ALOGI("ppt, in RtpDemuxer::ResolveSink, go to ResolveSinkByPayloadType.\n");
