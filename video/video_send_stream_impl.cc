...
std::vector<RtpRtcp*> CreateRtpRtcpModules(
  ...
  for (uint32_t ssrc : config.rtp.ssrcs) {
  	RTC_LOG(LS_ERROR) << "ppt, in CreateRtpRtcpModules, ssrc:" << ssrc;
...
VideoSendStreamImpl::VideoSendStreamImpl(
  ...
  RTC_LOG(LS_ERROR) << "ppt, in VideoSendStreamImpl::VideoSendStreamImpl, go to SetSink.\n";
...
EncodedImageCallback::Result VideoSendStreamImpl::OnEncodedImage(
  ...
  if (config_->post_encode_callback) {
  	RTC_LOG(LS_ERROR) << "ppt, in VideoSendStreamImpl::OnEncodedImage, go to EncodedFrameCallback";
  ...
  //RTC_LOG(LS_ERROR) << "ppt, in VideoSendStreamImpl::OnEncodedImage, go to payload_router_.OnEncodedImage";
