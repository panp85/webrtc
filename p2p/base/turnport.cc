...
bool TurnPort::CreateTurnClientSocket() {
  ...
  ALOGI("ppt, in CreateTurnClientSocket, server_address_.proto: %d.\n", server_address_.proto);
  if (server_address_.proto == PROTO_UDP && !SharedSocket()) {
  	ALOGI("ppt, in CreateTurnClientSocket, PROTO_UDP.\n");
    socket_ = socket_factory()->CreateUdpSocket(
        rtc::SocketAddress(Network()->GetBestIP(), 0), min_port(), max_port());
  } else if (server_address_.proto == PROTO_TCP ||
             server_address_.proto == PROTO_TLS) {
             ALOGI("ppt, in CreateTurnClientSocket, PROTO_TCP or PROTO_TLS.\n");
...
void TurnPort::DispatchPacket(const char* data, size_t size,
  ...
  if (Connection* conn = GetConnection(remote_addr)) {
  	ALOGI("ppt, in TurnPort::DispatchPacket, go to Connection::OnReadPacket.\n");
    conn->OnReadPacket(data, size, packet_time);
  } else {
  	ALOGI("ppt, in TurnPort::DispatchPacket, go to Port::OnReadPacket.\n");
