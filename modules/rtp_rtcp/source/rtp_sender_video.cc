...
bool RTPSenderVideo::SendVideo(enum VideoCodecType video_type,
  ...
  for (size_t i = 0; i < num_packets; ++i) {
    ...
    if (flexfec_enabled()) {
	  RTC_LOG(LS_ERROR) << "ppt, in RTPSenderVideo::SendVideo, go to SendVideoPacketWithFlexfec";
	  ...
	  //RTC_LOG(LS_ERROR) << "ppt, in RTPSenderVideo::SendVideo, go to SendVideoPacketAsRedMaybeWithUlpfec";
      SendVideoPacketAsRedMaybeWithUlpfec(std::move(packet), storage,
                                          protect_packet);
    } else {
	  RTC_LOG(LS_ERROR) << "ppt, in RTPSenderVideo::SendVideo, go to SendVideoPacket";
