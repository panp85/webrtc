...
void RtpFrameReferenceFinder::ManageFrame(
  ...
  switch (decision) {
    case kStash:
	  ALOGI("ppt, in RtpFrameReferenceFinder::ManageFrame kStash, go to stashed_frames_.push_front.\n");
      if (stashed_frames_.size() > kMaxStashedFrames)
        stashed_frames_.pop_back();
      stashed_frames_.push_front(std::move(frame));
      break;
    case kHandOff:
      ALOGI("ppt, in RtpFrameReferenceFinder::ManageFrame kHandOff, go to frame_callback_->OnCompleteFrame.\n");
  ...
  do {
    ...
    for (auto frame_it = stashed_frames_.begin();
      ...
      switch (decision) {
        ...
        case kHandOff:
          complete_frame = true;
		  ALOGI("ppt, in RtpFrameReferenceFinder::RetryStashedFrames, go to frame_callback_->OnCompleteFrame.\n");
