...
void Port::OnReadPacket(
  ...
  if (enable_port_packets_) {
  	ALOGI("ppt, in Port::OnReadPacket, go to SignalReadPacket.\n");
...
void Connection::OnReadPacket(
  ...
  if (!port_->GetStunMessage(data, size, addr, &msg, &remote_ufrag)) {
	...
	ALOGI("ppt, in Connection::OnReadPacket, go to SignalReadPacket.\n");
...
void Connection::Ping(int64_t now) {
  last_ping_sent_ = now;
  ALOGI("ppt, in Connection::Ping, go to new ConnectionRequest.\n");
...
int ProxyConnection::Send(const void* data, size_t size,
  ...
  RTC_LOG(LS_ERROR) << "ppt, in ProxyConnection::Send, go to port_->SendTo, remote_candidate_: " << remote_candidate_.ToString();
