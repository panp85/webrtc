...
bool VCMDecoderDataBase::RegisterReceiveCodec(const VideoCodec* receive_codec,
  ...
  ALOGE_PP("ppt, in VCMDecoderDataBase::RegisterReceiveCodec, receive_codec->plType: %d.\n", receive_codec->plType);
...
VCMGenericDecoder* VCMDecoderDataBase::GetDecoder(
  ...
  //if(receive_codec_){
	  ALOGE_PP("ppt, in VCMDecoderDataBase::GetDecoder, payload_type = %d, receive_codec_.plType = %d.\n", 
	  	payload_type, receive_codec_.plType);
  //}
 
  if (payload_type == receive_codec_.plType || payload_type == 0) {
    return ptr_decoder_.get();
  }
   ALOGE_PP("ppt, in VCMDecoderDataBase::GetDecoder 2.\n");
...
std::unique_ptr<VCMGenericDecoder> VCMDecoderDataBase::CreateAndInitDecoder(
  ...
  ALOGE_PP("ppt, in VCMDecoderDataBase::CreateAndInitDecoder, frame.PayloadType: %d.\n", (int)frame.PayloadType());
  uint8_t payload_type = frame.PayloadType();
  ...
  ALOGE_PP("ppt, in VCMDecoderDataBase::CreateAndInitDecoder, go to InitDecode.\n");
  ...
  //ALOGI("ppt, in VCMDecoderDataBase::CreateAndInitDecoder, new_codec: %s.\n", (new_codec)->name.c_str());						  
  return ptr_decoder;
}
