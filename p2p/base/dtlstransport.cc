...
void DtlsTransport::OnReadPacket(rtc::PacketTransportInternal* transport,
  ...
  if (!dtls_active_) {
    // Not doing DTLS.
    ALOGI("ppt, in DtlsTransport::OnReadPacket, go to SignalReadPacket 1.\n");
  ...
  switch (dtls_state()) {
    ...
    case DTLS_TRANSPORT_CONNECTED:
      ...
      if (IsDtlsPacket(data, size)) {
        ...
        // Sanity check.
        RTC_DCHECK(!srtp_ciphers_.empty());
		ALOGI("ppt, in DtlsTransport::OnReadPacket, go to SignalReadPacket 2.\n");
  ...
  if (sig & rtc::SE_READ) {
    ...
    do {
      ...
      if (ret == rtc::SR_SUCCESS) {
	  	ALOGI("ppt, in DtlsTransport::OnDtlsEvent, go to SignalReadPacket.\n");
...
bool DtlsTransport::HandleDtlsPacket(const char* data, size_t size) {
  ...
  ALOGI("ppt, in DtlsTransport::HandleDtlsPacket, go to downward_->OnPacketReceived.\n");
