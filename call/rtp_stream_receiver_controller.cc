...
#define ALOGI(...) __android_log_print(ANDROID_LOG_INFO, "logcat rtp_stream_receiver_controller.cc", __VA_ARGS__)
...
RtpStreamReceiverController::Receiver::Receiver(
  ...
  ALOGI("ppt, in RtpStreamReceiverController::Receiver::Receiver, go to controller_->AddSink.\n");
...
RtpStreamReceiverController::CreateReceiver(
    uint32_t ssrc,
    RtpPacketSinkInterface* sink) {
  ALOGI("ppt, in RtpStreamReceiverController::CreateReceiver, ssrc: %u, sink: %p", ssrc, sink);
  return rtc::MakeUnique<Receiver>(this, ssrc, sink);
}

bool RtpStreamReceiverController::OnRtpPacket(const RtpPacketReceived& packet) {
  rtc::CritScope cs(&lock_);
  ALOGI("ppt, in RtpStreamReceiverController::OnRtpPacket, go to demuxer_->OnRtpPacket.\n");
...
bool RtpStreamReceiverController::AddSink(uint32_t ssrc,
                                          RtpPacketSinkInterface* sink) {
  rtc::CritScope cs(&lock_);
  ALOGI("ppt, in RtpStreamReceiverController::AddSink, go to demuxer_->AddSink.\n");
