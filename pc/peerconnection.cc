...
//#define ALOGI(...) __android_log_print(ANDROID_LOG_INFO, "logcat peerconnection.cc", __VA_ARGS__)
...
bool PeerConnection::Initialize(
  ...
  if (!IsUnifiedPlan()) {
  	RTC_LOG(LS_ERROR)<<"ppt, !IsUnifiedPlan, go to transceivers_.push_back.";
...
rtc::scoped_refptr<RtpSenderInterface> PeerConnection::AddTrack(
  ...
  RTC_LOG(LS_ERROR) << "ppt, in PeerConnection::AddTrack 2, go to AddTrack";
...
RTCErrorOr<rtc::scoped_refptr<RtpSenderInterface>> PeerConnection::AddTrack(
  ...
  if (sender_or_error.ok()) {
    observer_->OnRenegotiationNeeded();
	RTC_LOG(LS_ERROR) << "ppt, in PeerConnection::AddTrack 1, go to stats_->AddTrack";
...
PeerConnection::AddTrackUnifiedPlan(
  ...
  if (transceiver) {
	...
	RTC_LOG(LS_ERROR) << "ppt, in PeerConnection::AddTrackUnifiedPlan, go to CreateAndAddTransceiver";
...
PeerConnection::AddTransceiver(
  ...
  auto receiver = CreateReceiver(media_type, rtc::CreateRandomUuid());
  RTC_LOG(LS_ERROR) << "ppt, in PeerConnection::AddTransceiver, go to CreateAndAddTransceiver";
...
PeerConnection::CreateAndAddTransceiver(
  ...
  RTC_LOG(LS_ERROR)<<"ppt, in PeerConnection::CreateAndAddTransceiver, go to transceivers_.push_back.";
...
RTCError PeerConnection::ApplyRemoteDescription(
  ...
  if (IsUnifiedPlan()) {
    ...
    for (auto transceiver : now_receiving_transceivers) {
		RTC_LOG(LS_ERROR) << "ppt, in PeerConnection::ApplyRemoteDescription, go to stats_->AddTrack";
...
RTCError PeerConnection::UpdateTransceiverChannel(
  ...
  if (content.rejected) {
    ...
    if (!channel) {
      if (transceiver->media_type() == cricket::MEDIA_TYPE_AUDIO) {
	  	RTC_LOG(LS_ERROR)<<"ppt, in PeerConnection::UpdateTransceiverChannel, go to CreateVoiceChannel:" << content.name.c_str();
        channel = CreateVoiceChannel(content.name);
      } else {
        RTC_DCHECK_EQ(cricket::MEDIA_TYPE_VIDEO, transceiver->media_type());
		RTC_LOG(LS_ERROR)<<"ppt, in PeerConnection::UpdateTransceiverChannel, go to CreateVideoChannel: " << content.name.c_str();
      ...
      transceiver->internal()->SetChannel(channel);
	   ...
	   RTC_LOG(LS_ERROR)<<"ppt, in PeerConnection::UpdateTransceiverChannel, channel yes.\n";
  }
  return RTCError::OK();
}
...
PeerConnection::AssociateTransceiver(cricket::ContentSource source,
  ...
  if (source == cricket::CS_LOCAL) {
    ...
    if (!transceiver) {
	  ...
	  RTC_LOG(LS_ERROR) << "ppt, in PeerConnection::AssociateTransceiver, go to CreateAndAddTransceiver";
      transceiver = CreateAndAddTransceiver(sender, receiver);
      transceiver->internal()->set_direction(
          RtpTransceiverDirection::kRecvOnly);
    }
...
RTCError PeerConnection::CreateChannels(const SessionDescription& desc) {
  ...
  if (video && !video->rejected &&
      !GetVideoTransceiver()->internal()->channel()) {
    RTC_LOG(LS_ERROR) << "ppt, in PeerConnection::CreateChannels, go to CreateVideoChannel.\n";
...
cricket::VideoChannel* PeerConnection::CreateVideoChannel(
    const std::string& mid) {
  RTC_LOG(LS_ERROR) << "ppt, in PeerConnection::CreateVideoChannel, go to transport_controller_->GetRtpTransport, mid = " << mid;
  RtpTransportInternal* rtp_transport =
      transport_controller_->GetRtpTransport(mid);
  RTC_DCHECK(rtp_transport);
  RTC_LOG(LS_ERROR) << "ppt, in PeerConnection::CreateVideoChannel, go to channel_manager CreateVideoChannel.\n";
