...
void RelayServer::OnExternalPacket(
  ...
  RTC_LOG(LS_ERROR) << "ppt, in RelayServer::OnExternalPacket, Adding connection from x";
...
void RelayServer::HandleStunAllocate(
  ...
  binding->AddInternalConnection(int_conn);
   RTC_LOG(LS_ERROR) << "ppt, in P2PTransportChannel::HandleStunAllocate, Adding connection from y";
...
void RelayServer::HandleStunSend(
  ...
  if (!ext_conn) {
    ...
    ext_conn->binding()->AddExternalConnection(ext_conn);
	RTC_LOG(LS_ERROR) << "ppt, in P2PTransportChannel::HandleStunSend, Adding connection from z";
    AddConnection(ext_conn);
  }
