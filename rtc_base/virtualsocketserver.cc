...
void VirtualSocket::CompleteConnect(const SocketAddress& addr, bool notify) {
  ...
  remote_addr_ = addr;
  state_ = CS_CONNECTED;
  RTC_LOG(LS_ERROR) << "ppt, in VirtualSocket::CompleteConnect, Adding connection from ddd";
...
AsyncSocket* VirtualSocketServer::CreateAsyncSocket(int family, int type) {
  ALOGI("ppt, in VirtualSocketServer::CreateAsyncSocket, go to CreateSocketInternal.\n");
  return CreateSocketInternal(family, type);
}
