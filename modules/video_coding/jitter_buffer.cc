  ...
  switch (buffer_state) {
    ...
    case kDecodableSession: {
      *retransmitted = (frame->GetNackCount() > 0);
      if (continuous) {
	  	ALOGI("ppt, VCMJitterBuffer::InsertPacket, go to decodable_frames_.InsertFrame.\n");
        decodable_frames_.InsertFrame(frame);
        FindAndInsertContinuousFrames(*frame);
      } else {
	  	ALOGI("ppt, VCMJitterBuffer::InsertPacket, go to incomplete_frames_.InsertFrame 1.\n");
    ...
    case kIncomplete: {
      if (frame->GetState() == kStateEmpty &&
        ...
        return kNoError;
	  	...
	  	ALOGI("ppt, VCMJitterBuffer::InsertPacket, go to incomplete_frames_.InsertFrame 2.\n");
        incomplete_frames_.InsertFrame(frame);
        // If NACKs are enabled, keyframes are triggered by |GetNackList|.
        if (nack_mode_ == kNoNack &&
            NonContinuousOrIncompleteDuration() >
    ...
    case kDuplicatePacket: {
      ...
      if (frame_list != NULL) {
	  	ALOGI("ppt, VCMJitterBuffer::InsertPacket, go to frame_list.InsertFrame.\n");
...
void VCMJitterBuffer::FindAndInsertContinuousFramesWithState(
  ...
  for (FrameList::iterator it = incomplete_frames_.begin();
    ...
    if (IsContinuousInState(*frame, decoding_state)) {
	  ALOGI("ppt, VCMJitterBuffer::FindAndInsertContinuousFramesWithState, go to decodable_frames_.InsertFrame.\n");
      decodable_frames_.InsertFrame(frame);
