...
AudioReceiveStream::AudioReceiveStream(
  ...
  ALOGI("ppt, in AudioReceiveStream::AudioReceiveStream, go to CreateReceiver.\n");
...
void AudioReceiveStream::SetSink(AudioSinkInterface* sink) {
  RTC_DCHECK_RUN_ON(&worker_thread_checker_);
  ALOGE_PP("ppt, in AudioReceiveStream::SetSink, go to SetSink.\n");
...
void AudioReceiveStream::OnRtpPacket(const RtpPacketReceived& packet) {
  ...
  // thread. Then this check can be enabled.
  // RTC_DCHECK(!thread_checker_.CalledOnValidThread());
  ALOGI("ppt, in AudioReceiveStream::OnRtpPacket, go to channel_proxy_->OnRtpPacket.\n");
