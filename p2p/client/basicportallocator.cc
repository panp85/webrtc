...
void AllocationSequence::OnMessage(rtc::Message* msg) {
  ...
  ALOGI("ppt, in AllocationSequence::OnMessage, phase_ = %d.\n", phase_);
  switch (phase_) {
    case PHASE_UDP:
      CreateUDPPorts();
      CreateStunPorts();
...
void AllocationSequence::OnReadPacket(
  ...
  for (auto* port : relay_ports_) {
    if (port->CanHandleIncomingPacketsFrom(remote_addr)) {
	  ALOGI("ppt, in AllocationSequence::OnReadPacket, go to HandleIncomingPacket 1.\n");
  ...
  if (udp_port_) {
    ...
    if (!turn_port_found ||
	  ...
	  ALOGI("ppt, in AllocationSequence::OnReadPacket, go to HandleIncomingPacket 2.\n");
