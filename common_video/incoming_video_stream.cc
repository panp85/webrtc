...
class IncomingVideoStream::NewFrameTask : public rtc::QueuedTask {
 ...
 private:
  bool Run() override {
    ...
    {
      //ALOGI("ppt, in IncomingVideoStream::NewFrameTask::Run, addFrame: 1, go to stream_->Dequeue.\n");
      stream_->Dequeue();
    }
	else
	  ALOGI("ppt, in IncomingVideoStream::NewFrameTask::Run, addFrame: not 1, return.\n");
    return true;
  }
