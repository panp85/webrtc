...
void RelayPort::OnReadPacket(
  ...
  if (Connection* conn = GetConnection(remote_addr)) {
  	ALOGI("ppt, in RelayPort::OnReadPacket 1, go to conn->OnReadPacket.\n");
    conn->OnReadPacket(data, size, packet_time);
  } else {
  	ALOGI("ppt, in RelayPort::OnReadPacket 1, go to Port::OnReadPacket.\n");
...
int RelayConnection::Send(const void* pv, size_t cb,
                          const rtc::PacketOptions& options) {
  RTC_LOG(LS_ERROR) << "ppt, in RelayConnection::Send, go to socket_->SendTo";
...
void RelayEntry::OnReadPacket(
  ...
  if (!port_->HasMagicCookie(data, size)) {
    if (locked_) {
	  ALOGI("ppt, in RelayPort::OnReadPacket 2, go to conn->OnReadPacket.\n");
  ...
  ALOGI("ppt, in RelayPort::OnReadPacket 3, go to conn->OnReadPacket.\n");
