...
void AudioRtpReceiver::SetStreams(
  ...
  for (auto stream : streams) {
    ...
    if (added) {
		RTC_LOG(LS_ERROR) << "ppt, in AudioRtpReceiver::SetStreams, go to stream->AddTrack";
...
bool VideoRtpReceiver::SetSink(rtc::VideoSinkInterface<VideoFrame>* sink) {
  RTC_DCHECK(media_channel_);
  RTC_DCHECK(ssrc_);
  ALOGE_PP("ppt, in VideoRtpReceiver::SetSink, go to SetSink.\n");
...
void VideoRtpReceiver::SetupMediaChannel(uint32_t ssrc) {
  ...
  ssrc_ = ssrc;
  ALOGE_PP("ppt, in VideoRtpReceiver::SetupMediaChannel, go to SetSink.\n");
...
void VideoRtpReceiver::SetStreams(
  ...
  for (auto stream : streams) {
    ...
    if (added) {
		RTC_LOG(LS_ERROR) << "ppt, in VideoRtpReceiver::SetStreams, go to stream->AddTrack";
      stream->AddTrack(track_);
    }
  }
  streams_ = streams;
