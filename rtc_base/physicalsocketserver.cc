...
#define ALOGI(...) __android_log_print(ANDROID_LOG_INFO, "logcat physicalsocketserver.cc", __VA_ARGS__)
...
int PhysicalSocket::Bind(const SocketAddress& bind_addr) {
  ...
  // model.
  ALOGI("ppt, in PhysicalSocket::Bind, go in.\n");
...
int PhysicalSocket::SendTo(const void* buffer,
  ...
  int sent = DoSendTo(
      ...
      struct sockaddr_in *sock = ( struct sockaddr_in*)(reinterpret_cast<sockaddr*>(&saddr));
	  struct in_addr in  = sock->sin_addr;
	  //char *ip1 = inet_ntoa(in);	 //注意此时ip1的地址不允许访问，会出错。
	  char str[INET_ADDRSTRLEN];   //INET_ADDRSTRLEN这个宏系统默认定义 16
	  //成功的话此时IP地址保存在str字符串中。
...
void SocketDispatcher::OnEvent(uint32_t ff, int err) {
  ...
  // something like a READ followed by a CONNECT, which would be odd.
  if ((ff & DE_CONNECT) != 0) {
  	ALOGI("ppt, in SocketDispatcher::OnEvent DE_CONNECT, go to SignalReadEvent.\n");
    DisableEvents(DE_CONNECT);
    SignalConnectEvent(this);
  }
  if ((ff & DE_ACCEPT) != 0) {
  	ALOGI("ppt, in SocketDispatcher::OnEvent DE_ACCEPT, go to SignalReadEvent.\n");
    DisableEvents(DE_ACCEPT);
    SignalReadEvent(this);
  }
  if ((ff & DE_READ) != 0) {
  	ALOGI("ppt, in SocketDispatcher::OnEvent DE_READ, go to SignalReadEvent.\n");
    DisableEvents(DE_READ);
    SignalReadEvent(this);
  }
  if ((ff & DE_WRITE) != 0) {
  	ALOGI("ppt, in SocketDispatcher::OnEvent DE_WRITE, go to SignalReadEvent.\n");
    DisableEvents(DE_WRITE);
    SignalWriteEvent(this);
  }
  if ((ff & DE_CLOSE) != 0) {
...
AsyncSocket* PhysicalSocketServer::CreateAsyncSocket(int family, int type) {
  ALOGI("ppt, in PhysicalSocketServer::CreateAsyncSocket, go in.\n");
