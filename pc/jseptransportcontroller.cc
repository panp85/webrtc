...
//#define ALOGI(...) __android_log_print(ANDROID_LOG_INFO, "logcat jseptransportcontroller.cc", __VA_ARGS__)
...
RTCError JsepTransportController::SetRemoteDescription(
    ...
    const cricket::SessionDescription* description) {
  if (!network_thread_->IsCurrent()) {
  	ALOGI("ppt, in JsepTransportController::SetRemoteDescription, go to SetRemoteDescription.\n");
    return network_thread_->Invoke<RTCError>(
        RTC_FROM_HERE, [=] { return SetRemoteDescription(type, description); });
  }
  ALOGI("ppt, in JsepTransportController::SetRemoteDescription, go to ApplyDescription_n.\n");
...
JsepTransportController::CreateDtlsTransport(const std::string& transport_name,
  ...
  //if(transport_name)
  RTC_LOG(LS_ERROR) << "ppt, in JsepTransportController::CreateDtlsTransport, transport_name: " << transport_name.c_str();
  std::unique_ptr<cricket::DtlsTransportInternal> dtls;
  if (config_.external_transport_factory) {
  	RTC_LOG(LS_ERROR) << "ppt, in JsepTransportController::CreateDtlsTransport, config_.external_transport_factory yes";
    ...
    dtls = config_.external_transport_factory->CreateDtlsTransport(
  	...
  	RTC_LOG(LS_ERROR) << "ppt, in JsepTransportController::CreateDtlsTransport, config_.external_transport_factory no";
...
JsepTransportController::CreateUnencryptedRtpTransport(
  ...
  RTC_LOG(LS_ERROR) << "ppt, in JsepTransportController::CreateUnencryptedRtpTransport, go to SetRtpPacketTransport.";
...
JsepTransportController::CreateSdesTransport(
  ...
  RTC_LOG(LS_ERROR) << "ppt, in JsepTransportController::CreateSdesTransport, go to SetRtpPacketTransport.";
...
RTCError JsepTransportController::MaybeCreateJsepTransport(
  ...
  RTC_LOG(LS_ERROR) << "ppt, in JsepTransportController::MaybeCreateJsepTransport, content_info.name: " << content_info.name;
