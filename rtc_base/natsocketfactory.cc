...
#define ALOGI(...) __android_log_print(ANDROID_LOG_INFO, "logcat natsocketfactory.cc", __VA_ARGS__)
...
class NATSocket : public AsyncSocket, public sigslot::has_slots<> {
 public:
  ...
  ALOGI("ppt, in NATSocket Bind, go in.\n");
    if (socket_) {  // already bound, bubble up error
      return -1;
    }
	ALOGI("ppt, in NATSocket Bind.\n");
...
AsyncSocket* NATSocketFactory::CreateAsyncSocket(int family, int type) {
  ALOGI("ppt, in NATSocketFactory::CreateAsyncSocket 1, go to new NATSocket.\n");
  return new NATSocket(this, family, type);
}
...
AsyncSocket* NATSocketServer::CreateAsyncSocket(int family, int type) {
  ALOGI("ppt, in NATSocketServer::CreateAsyncSocket 2, go to new NATSocket.\n");
  return new NATSocket(this, family, type);
}
