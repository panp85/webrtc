...
#define ALOGI(...) __android_log_print(ANDROID_LOG_INFO, "logcat videocapture.cc", __VA_ARGS__)
...
const std::vector<VideoFormat>* VideoCapturer::GetSupportedFormats() const {
  ALOGI("ppt, in VideoCapturer::GetSupportedFormats, go in.\n");
...
void VideoCapturer::AddOrUpdateSink(
  ...
  ALOGI("ppt, in VideoCapturer::AddOrUpdateSink, go in.\n");
  RTC_DCHECK(thread_checker_.CalledOnValidThread());
